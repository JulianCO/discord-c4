...
use self::connect4::board::{Board, GameStatus, Player, GameResult};
...
use discord::model::{ChannelId, Event, Message, MessageId, UserId, ReactionEmoji};
use discord::Discord;
use rand;
...
const COLUMN_EMOJI : [&str;7] = ["1️⃣", "2️⃣", "3️⃣", "4️⃣", "5️⃣", "6️⃣", "7️⃣"];
                        ...
                        for (i, emoji) in COLUMN_EMOJI.iter().enumerate() {
                            if u == emoji {
                                let responses = process_request(
                                    &mut conn, 
                                    &Request::RespondToInteraction(reaction.user_id, reaction.message_id, i as u8)
                                );
                                communicate_responses(&mut conn, &discord, reaction.channel_id, &responses);
                            }
                        }
                    }
                }
...
fn process_request(conn: &mut Connection, request: &Request) -> Vec<Response> {
    match request {
        ...
        Request::Challenge(channel, challenger, challenged, play_order) => {
            challenge_human(conn, channel, challenger, challenged, decide_random_order(*play_order))
        }
        ...
        Request::RespondToInteraction(player_id, message_id, move_no) => {
            let found_interaction = persistency::search_interaction(&conn, message_id.0, player_id.0);
            match found_interaction {
                Err(_) =>
                    vec![],
                Ok(ongoing_match) =>
                    process_request(conn, &Request::PlayMove(ChannelId(ongoing_match.get_server_id()), *player_id, *move_no))
            }
        }
...
fn process_move_vs_human(
    ...
    if !turn_ok {
        ...
        let mut responses = vec![Response::ShowGame(OngoingMatch::HumanMatch(human_match), setup_interaction)];
    ...
    if !turn_ok {
        ...
        let mut bot_responses = match computer_match.board.game_status() {
            GameStatus::GameOver(GameResult::Winner(_)) => {
                ...
                vec![Response::ShowComputerMatchOver(player_id, true, GameOverReason::PlayerWon)]
            }
            GameStatus::GameOver(GameResult::Tie) => {
                persistency::delete_match(conn, computer_match.match_id);
                vec![Response::ShowComputerMatchOver(player_id, true, GameOverReason::Tie)]
            }
            GameStatus::Turn(_) => {
                play_bot_move(conn, &computer_match)
            }
        };
        
        let mut responses = vec![Response::ShowGame(OngoingMatch::ComputerMatch(computer_match), false)];
...
fn challenge_human(
    ...
    match match_id_result {
        Err(Error::NotCompleted(NotCompletedReason::RedAlreadyPlaying)) => {
            vec![Response::ShowError(UserId(red_player_id), UserError::PlayerAlreadyPlaying)]
        }
        Err(Error::NotCompleted(NotCompletedReason::BlueAlreadyPlaying)) => {
            vec![Response::ShowError(UserId(blue_player_id), UserError::PlayerAlreadyPlaying)]
        }
    ...
    let responses = match bot_match_new.board.game_status() {
        GameStatus::GameOver(GameResult::Winner(_)) => {
            ...
            vec![
                ...
                Response::ShowComputerMatchOver(UserId(bot_match.player_id), false, GameOverReason::PlayerWon),
            ]
        }
        GameStatus::GameOver(GameResult::Tie) => {
            persistency::delete_match(conn, bot_match.match_id);
            vec![
                Response::AnnounceMove(suggested_move),
                Response::ShowGame(OngoingMatch::ComputerMatch(bot_match_new), false),
                Response::ShowComputerMatchOver(UserId(bot_match.player_id), true, GameOverReason::Tie),
            ]
        }
        GameStatus::Turn(_) => {
...
fn communicate_response(
    ...
    let response_string = match &response {
        ...
        Response::AnnounceMove(ai_move) => format!("I'm going to play in column {}", COLUMN_EMOJI[*ai_move as usize]),
        Response::ShowHumanMatchOver(_winner, _loser, _game_over_reason) => String::from("a game has ended"),
        Response::ShowComputerMatchOver(_player, _win, _game_over_reason) => String::from("a game has ended"),
        Response::ShowError(_user_id, _user_error) => String::from("an error has occured"),
    };
    let message = discord.send_message(
        channel_id,
        &response_string,
        "",
        false,
    ...
    if let Response::ShowGame(ongoing_match, true) = response {
        for (i, emoji) in COLUMN_EMOJI.iter().enumerate() {
            if ongoing_match.get_board().is_move_legal(i as u8) {
                ...
                match discord.add_reaction(channel_id, message.id, ReactionEmoji::Unicode(emoji.to_string())) {
...
fn check_player_turn_vs_bot(computer_match : &persistency::ComputerMatch, player_id : UserId) -> bool {
...
fn check_player_turn_vs_human(human_match : &persistency::HumanMatch, player_id : UserId) -> bool {
    match human_match.board.game_status() {
        GameStatus::Turn(Player::Red) =>
            player_id.0 == human_match.red_player_id,
        GameStatus::Turn(Player::Blue) =>
            player_id.0 == human_match.blue_player_id,
        _ => 
            false
    }
}
